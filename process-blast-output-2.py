...
if align == True:
    ...
    np.save(f"{directory}/msagen-{testprots[0][0]}", [prot_all, msagen])

    pdb2msa = msagen[0]
    msa2rind = msagen[1]

else:
    msagenprots = np.load(f"{directory}/msagen-{testprots[0][0]}.npy")
...
for x, prot in enumerate(prot_all):
    ...
    if ligand_resns_nonaq != []:
        ...
        if lining_rseqs_resis == False:
            ...
            print("error calculating pocket-lining resis")
            continue
        ...
        #print(lining_resis)
        #print(pdb2msa[prot])
        #print(len(pdb2msa[prot]))

        #print([pdb2msa[prot][i] for i in lining_resis]) #<--crashes
        #print(lining_resis)
        #print(pdb2msa[prot])
        #print(msa2rind[testprots[0][0]])
        lining_aligned = [msa2rind[prot_all[0]][pdb2msa[prot][i]] for i in lining_resis if pdb2msa[prot][i] in msa2rind[prot_all[0]]]

        if 115 in lining_aligned:
            print("--------------------------------------------------")
            print(lining_rseqs)
...
np.save(f"{directory}/outputs-{prot_all[0]}", [prot_all, lining_resis_byprot])

xtal = md.load(f"{directory}/rcsb_pdb/{prot_all[0]}.pdb")
