...
serial = 2 #for keeping track of saved output files
...
for i in structures:
    ...
    if i in hits_all: #skip MOAD structures previously encountered as BLAST hits
        print("structure previously encountered as a BLAST hit; skipping")
        continue
    ...
    for alignment in blast_record.alignments:
        ...
        #check that there is one hsp with at least idthresh x 100% identity and coverage
        if len(alignment.hsps) == 1 and hsp.identities/alignment.length > idthresh: #hsp.identities == hsp.align_length and hsp.align_length == alignment.length:
            print(alignment)
    ...
    hits_all += hits
    hits_alldata.append([hits, len(hits)])

    print(hits)

hits_alldata.sort(key = lambda x: x[1], reverse=True)
